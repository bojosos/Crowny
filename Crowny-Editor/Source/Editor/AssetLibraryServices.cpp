...
namespace Crowny
{
    ...
    void ImportScheduler::Schedule(Vector<ImportTask> tasks)
    {
        ...
        Vector<ImportTask> workerTasks;
        Vector<ImportTask> mainThreadTasks;
        for (ImportTask& task : tasks)
        {
            String extension = task.Entry->Filepath.extension().string();
            StringUtils::ToLower(extension);
            if (extension.size() > 1 && TextureImporter::IsExtensionSupportedStatic(extension.substr(1)))
                workerTasks.push_back(std::move(task));
            else
            {
                task.RunOnMainThread = true;
                mainThreadTasks.push_back(std::move(task));
            }
        }

        m_Progress.TotalFiles.store((uint32_t)(workerTasks.size() + mainThreadTasks.size()));
        m_Progress.CompletedFiles.store(0);
        m_Progress.Active.store(true);
        {
            Lock lock(m_Mutex);
            for (ImportTask& task : mainThreadTasks)
                m_Completed.push_back({ std::move(task), nullptr });
        }
        if (!workerTasks.empty())
            m_Worker = std::thread(&ImportScheduler::ImportWorker, this, std::move(workerTasks));
    }

    void ImportScheduler::ImportWorker(Vector<ImportTask> tasks)
    {
        for (const ImportTask& task : tasks)
        {
            ...
            {
                Lock lock(m_Mutex);
                m_Completed.push_back({ task, asset });
            }
    ...
    bool ImportScheduler::ProcessCompleted(const CompletionHandler& completionHandler, uint32_t maxPerFrame)
    {
        ...
        for (ImportResult& result : completed)
        {
            if (processed >= maxPerFrame)
            {
                deferred.push_back(std::move(result));
                continue;
            }
        ...
        {
            Lock lock(m_Mutex);
            m_Completed.insert(m_Completed.end(), std::make_move_iterator(deferred.begin()), std::make_move_iterator(deferred.end()));
            return false;
        }
        ...
        {
            Lock lock(m_Mutex);
            queueEmpty = m_Completed.empty();
        }
        const bool workerDone = !m_Worker.joinable() || m_Progress.CompletedFiles.load() >= m_Progress.TotalFiles.load();
