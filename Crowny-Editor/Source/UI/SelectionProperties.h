...
namespace Crowny::UI
{
    ...
    // These take `const Binding&` rather than a forwarding reference: the single-value
    // overloads in Properties.h are `template <typename TEnum> ...(TEnum& value)`, and for an
    // lvalue binding `TEnum&` would out-rank `Binding&&` in partial ordering and be selected.
    template <SelectionPropertyBinding Binding>
    static bool PropertyDropdown(const char* label, std::initializer_list<const char*> options, const Binding& property)
    {
        return EditSelectionProperty(property, [&](auto& value) { return PropertyDropdown(label, options, value); });
    }

    template <SelectionPropertyBinding Binding> static bool PropertyDropdown(const char* label, const Vector<String>& options, const Binding& property)
    {
        return EditSelectionProperty(property, [&](auto& value) { return PropertyDropdown(label, options, value); });
    }

    template <SelectionPropertyBinding Binding>
    static bool PropertyIconTabs(const char* label, std::initializer_list<PropertyIconTab> tabs, const Binding& property)
    {
        return EditSelectionProperty(property, [&](auto& value) { return PropertyIconTabs(label, tabs, value); });
    }
