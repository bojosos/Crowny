...
namespace Crowny
{
	...
	bool ScriptInspector::DrawListInspector(MonoObject* listObject, const Ref<SerializableMemberInfo>& memberInfo, std::function<void(void*)> setter, int depth)
	{
		...
		if (UI::PropertyInput(memberInfo->m_Name.c_str(), newLength))
		{
			...
			{
				MonoClass* klass = MonoManager::Get().FindClass(memberInfo->m_TypeInfo->GetMonoClass()); // So this somehow works without having to bind parameters
	...
	bool ScriptInspector::DrawEnumInspector(const Ref<SerializableMemberInfo>& memberInfo, const Ref<SerializableTypeInfoEnum>& enumInfo, std::function<MonoObject* ()> getter,
	...
	// draws it all.
	bool ScriptInspector::DrawObjectInspector(const Ref<SerializableObjectInfo>& objectInfo, MonoObject* instance, std::function<void(void*)> setter, int depth)
	{
