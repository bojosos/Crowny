...
namespace Crowny
{
    ...
    static void DrawPrimitiveInspector(const Ref<SerializableMemberInfo>& memberInfo,
                                       std::function<MonoObject*()> getter, std::function<void(void*)> setter,
                                       const Ref<SerializableTypeInfo>& listTypeInfo = nullptr);
    static void DrawFieldInspector(const Ref<SerializableMemberInfo>& memberInfo, std::function<MonoObject*()> getter,
                                       ...
                                       const Ref<SerializableTypeInfo>& listInfo)
    {
        ...
        case ScriptPrimitiveType::I8: {
            ...
            if (!memberInfo->m_Flags.IsSet(ScriptFieldFlagBits::Range))
                ...
                change =
                  ImGui::DragScalar("##dragInt8", ImGuiDataType_S8, &value, DRAG_SENSITIVITY, &minValue8, &maxValue8);
        ...
        case ScriptPrimitiveType::U8: {
            ...
            if (!memberInfo->m_Flags.IsSet(ScriptFieldFlagBits::Range))
                ...
                change =
                  ImGui::DragScalar("##dragUInt8", ImGuiDataType_U8, &value, DRAG_SENSITIVITY, &minValue8, &maxValue8);
            value = glm::clamp(value, minValue8, maxValue8);
            if (change)
                setter(&value);
            break;
        ...
        case ScriptPrimitiveType::I32: {
            ...
            if (!memberInfo->m_Flags.IsSet(ScriptFieldFlagBits::Range))
                ...
                change = ImGui::DragScalar("##dragInt32", ImGuiDataType_S32, &value, DRAG_SENSITIVITY, &minValue32,
                                           &maxValue32);
        ...
        case ScriptPrimitiveType::U32: {
            ...
            if (!memberInfo->m_Flags.IsSet(ScriptFieldFlagBits::Range))
                ...
                change = ImGui::DragScalar("##dragUInt32", ImGuiDataType_U32, &value, DRAG_SENSITIVITY, &minValue32,
                                           &maxValue32);
            value = glm::clamp(value, minValue32, maxValue32);
            if (change)
                setter(&value);
        ...
        case ScriptPrimitiveType::I64: {
            ...
            if (!memberInfo->m_Flags.IsSet(ScriptFieldFlagBits::Range))
                ...
                change = ImGui::DragScalar("##dragInt64", ImGuiDataType_S64, &value, DRAG_SENSITIVITY, &minValue64,
                                           &maxValue64);
        ...
        case ScriptPrimitiveType::U64: {
            ...
            if (!memberInfo->m_Flags.IsSet(ScriptFieldFlagBits::Range))
                ...
                change = ImGui::DragScalar("##dragUInt64", ImGuiDataType_U64, &value, DRAG_SENSITIVITY, &minValue64,
                                           &maxValue64);
            value = glm::clamp(value, minValue64, maxValue64);
            if (change)
                setter(&value);
    ...
    static void DrawEnumInspector(const Ref<SerializableTypeInfoEnum>& enumInfo, std::function<MonoObject*()> getter,
                                  std::function<void(void*)> setter)
    {
        uint32_t value =
                                   ...
                                   int depth)
    {
        ...
        else if (typeInfo->GetType() == SerializableType::Entity)
        {
            ...
            if (ImGui::Button(entityName.c_str(),
                              ImVec2(ImGui::GetContentRegionAvailWidth() * 0.69f, 0))) // Open entity selection modal
                                                                   ...
                                                                   objTypeInfo->m_TypeName, objInfo))
            {
                ...
                const MonoScriptComponent& comp =
                  Entity(*scene->GetAllEntitiesWith<MonoScriptComponent>().begin(), scene.get())
                    .GetComponent<MonoScriptComponent>();
                ...
                if (fieldClass->IsSubClassOf(ScriptInfoManager::Get().GetBuiltinClasses().EntityClass)) // Entity handle
                {
                    ...
                    if (ImGui::BeginDragDropTarget()) // Drag entities on the control
                    {
                        if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("Entity_ID"))
                        {
                            ...
                            Entity entity{ (entt::entity)id, SceneManager::GetActiveScene().get() };
                            MonoObject* value =
           ScriptSceneObjectManager::Get().GetOrCreateScriptEntity(entity)->GetManagedInstance(); field->Set(instance,
           value);
                        }
                        ImGui::EndDragDropTarget();
                    }
