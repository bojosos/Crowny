...
TEST_CASE("Clustered lights assign local lights and separate directionals", "[Renderer][Lights][Clusters]")
{
    ...
    ClusteredLightBuilder::Build(desc, glm::mat4(1.0f), glm::perspective(glm::radians(60.0f), 1.0f, 0.1f, 100.0f),
                                 lights.data(), static_cast<uint32_t>(lights.size()), grid);
...
TEST_CASE("Clustered light overflow is explicit and bounded", "[Renderer][Lights][Clusters]")
{
    ...
    ClusteredLightGrid grid;
    ClusteredLightBuilder::Build(desc, glm::mat4(1.0f), glm::perspective(glm::radians(60.0f), 1.0f, 0.1f, 10.0f),
                                 lights.data(), static_cast<uint32_t>(lights.size()), grid);
