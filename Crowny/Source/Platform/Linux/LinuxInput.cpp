...
namespace Crowny
{
	...
	static std::unordered_map<KeyCode, bool> s_KeyStates(0);
	static std::unordered_map<MouseCode, bool> s_MouseStates(0);

	static std::vector<KeyCode> s_Keys = {
		Key::Space,
		Key::Apostrophe,
		Key::Comma,
		Key::Minus,
	...
	static std::vector<MouseCode> s_MouseButtons =
	{
		Mouse::Button0,
		Mouse::Button1,
		Mouse::Button2,
	...
	{
		return GetKey(key) && !s_KeyStates[key];
	}
	...
	{
		return GetMouseButton(btn);
	}

	bool Input::IsMouseButtonDown(const MouseCode btn)
	{
		return GetMouseButton(btn) && !s_MouseStates[btn];
	}

	bool Input::IsMouseButtonUp(const MouseCode btn)
	{
		return !GetMouseButton(btn) && s_MouseStates[btn];
	}
