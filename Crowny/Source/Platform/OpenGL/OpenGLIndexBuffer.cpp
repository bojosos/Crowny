...
namespace Crowny
{
    ...
    OpenGLIndexBuffer::OpenGLIndexBuffer(uint32_t count, IndexType indexType, BufferUsage usage)
      : m_Count(count), m_IndexType(indexType)
    {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * indexSize, nullptr,
                     OpenGLUtils::BufferUsageToOpenGLBufferUsage(usage));
#else
      ...
      : m_Count(count), m_IndexType(IndexType::Index_16)
    {
        ...
        glGenBuffers(1, &m_RendererID);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_RendererID);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * sizeof(uint16_t), indices,
                     OpenGLUtils::BufferUsageToOpenGLBufferUsage(usage));
#else
        glCreateBuffers(1, &m_RendererID);
        glNamedBufferData(m_RendererID, count * sizeof(uint16_t), indices,
                          OpenGLUtils::BufferUsageToOpenGLBufferUsage(usage));
#endif
    }
    ...
    void* OpenGLIndexBuffer::Map(uint32_t offset, uint32_t size, GpuLockOptions options)
    {
        ...
        case (GpuLockOptions::READ_ONLY):
            return glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, offset, size, GL_MAP_READ_BIT);
        case (GpuLockOptions::WRITE_ONLY):
            return glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, offset, size, GL_MAP_WRITE_BIT);
        case (GpuLockOptions::WRITE_DISCARD):
            return glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, offset, size,
                                    GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
        case (GpuLockOptions::WRITE_DISCARD_RANGE):
            return glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, offset, size,
                                    GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT);
        case (GpuLockOptions::READ_WRITE):
            return glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, offset, size, GL_MAP_WRITE_BIT | GL_MAP_READ_BIT);
