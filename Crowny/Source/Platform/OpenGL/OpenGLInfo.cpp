...
namespace Crowny
{
	...
	std::vector<OpenGLDetail>& OpenGLInfo::GetInformation()
	{
	...
	void OpenGLInfo::RetrieveInformation()
	{
		...
		s_Information.push_back({ "GLSL Version", "GL_SHADING_LANGUAGE_VERSION", std::string((char*)glGetString(GL_SHADING_LANGUAGE_VERSION)) });

		int maxTexture2DSize;
		glGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxTexture2DSize);
		s_Information.push_back({ "Max Texture2D Size", "GL_MAX_TEXTURE_SIZE", std::to_string(maxTexture2DSize) + "x" + std::to_string(maxTexture2DSize) });

		int maxTexture3DSize;
		glGetIntegerv(GL_MAX_3D_TEXTURE_SIZE, &maxTexture3DSize);
		s_Information.push_back({ "Max Texture3D Size", "GL_MAX_3D_TEXTURE_SIZE", std::to_string(maxTexture3DSize) + "x" + std::to_string(maxTexture3DSize) + "x" + std::to_string(maxTexture3DSize) });
