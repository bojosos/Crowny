...
namespace Crowny
{
        ...
        : m_PhysicalDevice(physicalDevice)
    {
        ...
        m_CommandBufferPool = new VulkanCommandBufferPool();
        ...
        deviceInfo.ppEnabledExtensionNames = m_SupportedExtensions;
    ...
    VulkanDevice::~VulkanDevice()
    {
        ...
        for (uint32_t i = 0; i < QUEUE_COUNT; i++)
        {
            ...
            {
                m_QueueInfos[i].Queues[j]->RefreshStates(true, true);
    ...
    SurfaceFormat VulkanDevice::GetSurfaceFormat(const VkSurfaceKHR& surface) const
    {
        ...
        VkResult result = vkGetPhysicalDeviceSurfaceFormatsKHR(m_Device, surface, &formatCount, nullptr);
        CW_ENGINE_ASSERT(result == VK_SUCCESS && formatCount > 0);
        
        std::vector<VkSurfaceFormatKHR> surfaceFormats(formatCount);
        result = vkGetPhysicalDeviceSurfaceFormatsKHR(m_Device, surface, &formatCount, surfaceFormats.data());
        CW_ENGINE_ASSERT(result == VK_SUCCESS);
