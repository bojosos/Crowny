...
namespace Crowny
{
	...
	bool VulkanQuery::GetResultArray(Vector<uint64_t>& result)
	{
		...
		result.resize(count);
		VkResult vkResult  = vkGetQueryPoolResults(vkDevice, m_Pool, m_QueryIdx, 1, result.size()  * sizeof(uint64_t), result.data(), sizeof(uint64_t), VK_QUERY_RESULT_64_BIT);
	...
	VulkanQueryPool::PoolInfo& VulkanQueryPool::AllocatePool(VkQueryType type)
	{
		...
		if (type == VK_QUERY_TYPE_PIPELINE_STATISTICS)
		{
			...
			{
				queryCount += 2;
				flags |= VK_QUERY_PIPELINE_STATISTIC_TESSELLATION_CONTROL_SHADER_PATCHES_BIT | VK_QUERY_PIPELINE_STATISTIC_TESSELLATION_EVALUATION_SHADER_INVOCATIONS_BIT;
			}
		}
		...
		Vector<PoolInfo>& poolInfos = type == VK_QUERY_TYPE_TIMESTAMP ? m_TimerPools : (type == VK_QUERY_TYPE_PIPELINE_STATISTICS ? m_PipelinePools : m_OcclusionPools);
		poolInfo.StartIdx = (uint32_t)poolInfos.size() * queryCount;

		poolInfos.push_back(poolInfo);
		Vector<VulkanQuery*>& queries = type == VK_QUERY_TYPE_TIMESTAMP ? m_TimerQueries : (type == VK_QUERY_TYPE_PIPELINE_STATISTICS ? m_PipelineQueries : m_OcclusionQueries);
	...
	void VulkanQueryPool::Release(VulkanQuery* query)
	{
		query->m_Free = true;
	}

	void VulkanQuery::Reset(VkCommandBuffer cmdBuffer)
	{
		vkCmdResetQueryPool(cmdBuffer, m_Pool, m_QueryIdx, 1);
	}
	...
	{
		return !m_Queries.empty() && !m_QueryEndCalled;
	}
