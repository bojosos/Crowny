...
namespace Crowny
{
    ...
    VulkanRenderPass* VulkanRenderPasses::GetRenderPass(const VulkanRenderPassDesc& desc) const
    {
        ...
        pass = new VulkanRenderPass(gVulkanRenderAPI().GetPresentDevice()->GetLogicalDevice(), desc);
        m_Passes[key] = pass;
        return pass;
    }

    VulkanRenderPasses::PassVariant::PassVariant(const VulkanRenderPassDesc& desc) : Desc(desc) {}
