...
namespace Crowny
{
    ...
    VulkanVertexBuffer::VulkanVertexBuffer(void* data, uint32_t size, const VertexBufferProperties& props)
        : m_Buffer(nullptr), m_StagingBuffer(nullptr), m_StagingMemory(nullptr), m_MappedOffset(0), m_MappedSize(0),
          m_MappedLockOptions(GpuLockOptions::WRITE_ONLY), m_DirectlyMappable(props.Usage == BufferUsage::DYNAMIC_DRAW),
        m_IsMapped(false), m_SupportsGpuWrites(false), m_Size(size)
    {
        auto& device = gVulkanRendererAPI().GetPresentDevice();
        
        VkBufferUsageFlags usageFlags = 0;
        usageFlags = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; // TODO: the other buffers
    ...
    VulkanBuffer* VulkanVertexBuffer::CreateBuffer(VulkanDevice& device, uint32_t size, bool staging, bool readable)
    {
    ...
    void* VulkanVertexBuffer::Map(uint32_t offset, uint32_t length, GpuLockOptions options)
    {
    ...
    void* VulkanVertexBuffer::GetPointer(uint32_t size)
    {
        return Map(0, size, GpuLockOptions::WRITE_DISCARD);
    }
    
    void VulkanVertexBuffer::FreePointer()
    {
        Unmap();
    }
