...
namespace Crowny
{
    ...
    VkFilter VulkanUtils::GetFilter(TextureFilter filter)
    {
        switch(filter)
        {
            ...
            case TextureFilter::ANISOTROPIC: return VK_FILTER_LINEAR;
            case TextureFilter::NEAREST:
            case TextureFilter::NONE: return VK_FILTER_NEAREST;
        }
    ...
    VkSamplerMipmapMode VulkanUtils::GetMipFilter(TextureFilter filter)
    {
        switch(filter)
        {
            ...
            case TextureFilter::NEAREST:
            case TextureFilter::NONE: return VK_SAMPLER_MIPMAP_MODE_NEAREST;
        }
    ...
    VkCompareOp VulkanUtils::GetCompareOp(CompareFunction compareFunc)
    {
        switch(compareFunc)
        {
            case CompareFunction::ALWAYS_FAIL:   return VK_COMPARE_OP_NEVER;
            case CompareFunction::ALWAYS_PASS:   return VK_COMPARE_OP_ALWAYS;
    ...
    VkCullModeFlagBits VulkanUtils::GetCullMode(CullingMode mode)
    {
        switch(mode)
        {
            case CullingMode::CULL_NONE:             return VK_CULL_MODE_NONE;
            case CullingMode::CULL_CLOCKWISE:        return VK_CULL_MODE_FRONT_BIT;
            case CullingMode::CULL_COUNTERCLOCKWISE: return VK_CULL_MODE_BACK_BIT;
        }
    ...
    VkBlendOp VulkanUtils::GetBlendOp(BlendFunction blendFunc)
    {
        switch(blendFunc)
        {
            ...
            case BlendFunction::REVERSE_SUBTRACT:   return VK_BLEND_OP_REVERSE_SUBTRACT;
    ...
    TextureFormat VulkanUtils::GetClosestSupportedTextureFormat(const VulkanDevice& device, TextureFormat format, TextureShape shape, int usage, bool optimalTiling)
    {
        ...
        if (!isSupported(vkFormat))
        {
            if ((usage & TEXTURE_DEPTHSTENCIL) != 0)
            {
                ...
                if (hasStencil)
                    {
                    if (isSupported(VK_FORMAT_D24_UNORM_S8_UINT)) // spec guarantees that at least on depth-stencil format is supported.
    ...
    VkFormat VulkanUtils::GetTextureFormat(TextureFormat format, bool sRGB)
    {
        switch(format)
        {
            case TextureFormat::R8: if (sRGB) return VK_FORMAT_R8_SRGB; return VK_FORMAT_R8_UNORM;
            case TextureFormat::RG8: if (sRGB) return VK_FORMAT_R8G8_SRGB; return VK_FORMAT_R8G8_UNORM;
            case TextureFormat::RGB8: if (sRGB) return VK_FORMAT_R8G8B8_SRGB; return VK_FORMAT_R8G8B8_UNORM;
            case TextureFormat::RGBA8: if (sRGB) return VK_FORMAT_R8G8B8A8_SRGB; return VK_FORMAT_R8G8B8A8_UNORM;
    ...
    bool VulkanUtils::RangeOverlaps(const VkImageSubresourceRange& a, const VkImageSubresourceRange& b)
    {
        ...
        if ((int32_t)a.baseArrayLayer < bRight && aRight > (int32_t)b.baseArrayLayer && (int32_t)a.baseMipLevel < bBottom && aBottom > (int32_t)b.baseMipLevel)
            return true;
        return false;
    }

    void VulkanUtils::CutRange(const VkImageSubresourceRange& a, const VkImageSubresourceRange& b, std::array<VkImageSubresourceRange, 5>& output, uint32_t& numAreas)
    {
    ...
    VkFormat VulkanUtils::GetDummyViewFormat(GpuBufferFormat format)
    {
        ...
        {
        case BF_16x1F:
        case BF_32x1F: return VK_FORMAT_R32_SFLOAT;
        case BF_16X2F:
        case BF_32x2F: return VK_FORMAT_R16G16_UNORM;
        case BF_32x3F:
        case BF_32x4F: return VK_FORMAT_R8G8B8A8_UNORM;

        case BF_16X1U:
        case BF_32X1U: return VK_FORMAT_R32_UINT;
            
        case BF_16X2U:
        case BF_32X2U: return VK_FORMAT_R16G16_UINT;
            
        case BF_16X4U:
        case BF_32X3U:
        case BF_32X4U: return VK_FORMAT_R8G8B8A8_UINT;
