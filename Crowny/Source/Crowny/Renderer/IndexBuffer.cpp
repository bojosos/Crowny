...
namespace Crowny
{
	...
	Ref<IndexBuffer> IndexBuffer::Create(uint32_t count, IndexType indexType, BufferUsage usage)
	{
		...
		{
			case RendererAPI::API::OpenGL: return CreateRef<OpenGLIndexBuffer>(count, indexType, usage);
			case RendererAPI::API::Vulkan: return CreateRef<VulkanIndexBuffer>(count, indexType, usage);
	...
	Ref<IndexBuffer> IndexBuffer::Create(uint16_t* indices, uint32_t count, BufferUsage usage)
	{
		switch (Renderer::GetAPI())
		{
			...
			case RendererAPI::API::Vulkan: return CreateRef<VulkanIndexBuffer>(indices, count, usage);
			default: 					   CW_ENGINE_ASSERT(false, "Renderer API not supporter"); return nullptr;
		}
