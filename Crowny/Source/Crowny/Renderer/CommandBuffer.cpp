...
namespace Crowny
{
	...
	uint32_t CommandSyncMask::GetGlobalQueueMask(GpuQueueType type, uint32_t queueIdx)
	{
		...
		switch(type)
		{
		case GpuQueueType::GRAPHICS_QUEUE: break;
		case GpuQueueType::COMPUTE_QUEUE: bitShift = 8; break;
		case GpuQueueType::UPLOAD_QUEUE: bitShift = 16; break;
		default: break;
		}
	...
	uint32_t CommandSyncMask::GetGlobalQueueIdx(GpuQueueType type, uint32_t queueIdx)
	{
		switch(type)
		{
			case GpuQueueType::COMPUTE_QUEUE: return 8 + queueIdx;
			case GpuQueueType::UPLOAD_QUEUE: return 16 + queueIdx;
			default: return queueIdx;
		}
	}
    ...
    Ref<CommandBuffer> CommandBuffer::Create(GpuQueueType type, uint32_t queueIdx, bool secondary)
    {
        switch (Renderer::GetAPI())
		{
			...
			case RendererAPI::API::Vulkan: return CreateRef<VulkanCommandBuffer>(*gVulkanRendererAPI().GetPresentDevice().get(), type, queueIdx, secondary);
