...
namespace Crowny
{
    ...
    public:
        ...
        void Apply(const RenderWorldChange* instanceChanges, uint32_t instanceChangeCount,
                   const RenderLightChange* lightChanges, uint32_t lightChangeCount);
        ...
        void Reset();
        void UploadShadowData(const GpuShadowLightData* lights, uint32_t lightCount,
                              const GpuShadowViewData* views, uint32_t viewCount);
        ...
        Ref<VertexBuffer> GetMeshVertexBuffer(uint32_t meshIndex) const;
        Ref<IndexBuffer> GetMeshIndexBuffer(uint32_t meshIndex) const;
        ...
        void BuildCpuDrawList(const RenderView& view, GpuDrawList& output,
                              GpuDrawBuffers* outputBuffers = nullptr, bool shadowCastersOnly = false);
    ...
    private:
        ...
        void UploadTable(Ref<GenericGpuBuffer>& buffer, const void* data, uint32_t elementCount, uint32_t elementSize,
                         uint32_t minimumCapacity, uint32_t& capacity, uint32_t& rangeCount);
        void UploadTableRanges(Ref<GenericGpuBuffer>& buffer, const void* data, uint32_t elementCount,
