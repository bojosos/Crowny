...
#define CW_INTERNAL_ASSERT_IMPL(type, check, msg, ...)                                                                                               \
    {                                                                                                                                                \
        if (!(check))                                                                                                                                \
        {                                                                                                                                            \
            CW##type##ERROR(msg, __VA_ARGS__);                                                                                                       \
            CW_DEBUGBREAK();                                                                                                                         \
        }                                                                                                                                            \
    }
...
#define CW_INTERNAL_ASSERT_NO_MSG(type, check)                                                                                                       \
    CW_INTERNAL_ASSERT_IMPL(type, check, "Assertion '{0}' failed at {1}:{2}", CW_STRINGIFY_MACRO(check), fs::path(__FILE__).filename().string(),     \
                            __LINE__)

#define CW_INTERNAL_ASSERT_GET_MACRO_NAME(arg1, arg2, macro, ...) macro
#define CW_INTERNAL_ASSERT_GET_MACRO(...)                                                                                                            \
    CW_EXPAND_MACRO(CW_INTERNAL_ASSERT_GET_MACRO_NAME(__VA_ARGS__, CW_INTERNAL_ASSERT_WITH_MSG, CW_INTERNAL_ASSERT_NO_MSG))
