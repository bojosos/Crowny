...
namespace Crowny
{
    ...
    glm::vec3 Math::GetRightDirection(const glm::vec3& rotation)
    {
        ...
        const float x = glm::cos(yaw);
        const float y = 0;
        const float z = glm::sin(yaw);
    ...
    void Transform::MakeWorld(const Transform& parentTransform)
    {
        ...
        const glm::mat4 localMatrix = GetMatrix();
    ...
    {
        const glm::mat4 invParent = glm::inverse(parentTransform.GetMatrix());
        const glm::vec4 localPos = invParent * glm::vec4(position, 1.0f);
        m_Position = glm::vec3(localPos);
    }

    void Transform::SetWorldRotation(const glm::quat& rotation, const Transform& parentTransform)
    {
        const glm::quat invParentRot = glm::inverse(parentTransform.GetRotation());
