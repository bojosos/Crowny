...
namespace Crowny
{
        ...
        switch(bitDepth)
        {
            case 8:
            {
                switch(numChannels)
                {
                    ...
                    case 4:  return alGetEnumValue("AL_FORMAT_QUEAD8");
                    ...
                    default: CW_ENGINE_ASSERT(false); return 0;
                }
            }
            case 16:
            {
                switch(numChannels)
                {
                    ...
                    case 4:  return alGetEnumValue("AL_FORMAT_QUEAD16");
    ...
    int32_t Convert24To32Bits(const uint8_t* in)
    {
        return (in[2] << 24) | (in[1] << 16) | (in[0] << 8);
    }
    ...
    void AudioUtils::ConvertBitDepth(const uint8_t* samples, uint32_t inBitDepth, uint8_t* output, uint32_t outBitDepth, uint32_t numSamples)
    {
        ...
        switch(inBitDepth)
        {
            case 8:  Convert8To32Bits((int8_t*)samples, src, numSamples); break;
            case 16: Convert16To32Bits((int16_t*)samples, src, numSamples); break;
            case 24: ::Crowny::Convert24To32Bits(samples, src, numSamples); break;
            case 32: break;
            default: CW_ENGINE_ASSERT(false); break;
        }
        
        switch(outBitDepth)
        {
        case 8: Convert32To8Bits(src, output, numSamples); break;
        case 16: Convert32To16Bits(src, output, numSamples); break;
        case 24: Convert32To24Bits(src, output, numSamples); break;
        case 32: std::memcpy(output, src, numSamples * sizeof(uint32_t)); break;
        default: CW_ENGINE_ASSERT(false); break;
        }
    ...
    void AudioUtils::ConvertToMono(const uint8_t* input, uint8_t* output, uint32_t bitDepth, uint32_t numSamples, uint32_t numChannels)
    {
        switch(bitDepth)
        {
            case 8:  ConvertToMono8((int8_t*)input, output, bitDepth, numSamples, numChannels); break;
            case 16: ConvertToMono16((int16_t*)input, (int16_t*)output, bitDepth, numSamples, numChannels); break;
            case 24: ConvertToMono24(input, output, bitDepth, numSamples, numChannels); break;
            case 32: ConvertToMono32((int32_t*)input, (int32_t*)output, bitDepth, numSamples, numChannels); break;
            default: CW_ENGINE_ASSERT(false); break;
        }
    }
