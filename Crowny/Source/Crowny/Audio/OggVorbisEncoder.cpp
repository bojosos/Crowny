...
namespace Crowny
{
	...
	bool OggVorbisEncoder::Open(std::function<void(uint8_t*, uint32_t)> writeCallback, uint32_t sampleRate, uint32_t bitDepth,
...
/*
    ...
    bool OggVorbisEncoder::Open(std::function<void(uint8_t*, uint32_t)> writeCallback, uint32_t sampleRate, uint32_t bitDepth, uint32_t numChannels, float quality = 1.0f)
    {
        m_NumChannels = numChannels;
        m_BitDepth = bitDepth;
        m_WriteCallback = writeCallback;
        ...
        int32_t status = vorbis_encode_init_vbr(&m_VorbisInfo, numChannels, sampleRate, quality); // Variable bitrate
        if (status != 0)
        {
