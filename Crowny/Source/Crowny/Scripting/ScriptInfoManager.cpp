...
namespace Crowny
{
	...
	ScriptInfoManager::ScriptInfoManager() { RegisterComponents(); RegisterAssets(); }
	...
	void ScriptInfoManager::LoadAssemblyInfo(const String& assemblyName)
	{
		...
		for (auto* klass : allClasses)
		{
			...
			const bool isSerializable =
				klass->IsSubClassOf(m_Builtin.ComponentClass) || klass->IsSubClassOf(assetClass) || klass->HasAttribute(m_Builtin.SerializableObjectAtrribute);
			...
			if ((isSerializable || isInspectable) &&
				klass != m_Builtin.ComponentClass && klass != assetClass)
			{
