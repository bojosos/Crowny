...
namespace Crowny
{
    ...
    CWMonoField::CWMonoField(MonoClassField* field) : m_Field(field)
    {
        ...
        m_FullDeclName =
          CWMonoVisibilityToString(GetVisibility()) + (IsStatic() ? " static " : " ") + mono_field_full_name(m_Field);

        if (IsStatic())
        {
            m_OwningTypeVTable =
              mono_class_vtable(CWMonoRuntime::GetDomain(), mono_type_get_class(mono_field_get_type(field)));
    ...
    void CWMonoField::Set(MonoObject* obj, void* value) { mono_field_set_value(obj, m_Field, value); }

    void CWMonoField::Get(MonoObject* obj, void* outval) { mono_field_get_value(obj, m_Field, outval); }
    ...
    bool CWMonoField::IsValueType() const { return m_Type->IsValueType(); }

    bool CWMonoField::IsStatic() const { return (mono_field_get_flags(m_Field) & MONO_FIELD_ATTR_STATIC) != 0; }
