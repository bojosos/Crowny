        ...
        public:
            ...
            void NotifySceneEventsAvailable() override
            {
                ...
                if (!m_RuntimeReady || m_Api.notify_scene_event == nullptr || manager == nullptr)
                    return;
                const Vector<SceneLifecycleEvent> events = manager->DrainManagedLifecycleEvents();
                ...
                for (const SceneLifecycleEvent& event : events)
                {
                    const cw_managed_status status =
                      m_Api.notify_scene_event(static_cast<uint32_t>(event.Type), ToAbiUuid(event.SceneId), static_cast<uint32_t>(event.State));
                    if (status == CW_MANAGED_STATUS_OK)
                        continue;
                    CollectManagedDiagnostics();
                    ManagedDiagnostic diagnostic;
                    diagnostic.Severity = ManagedDiagnosticSeverity::Error;
                    diagnostic.Code = "managed.coreclr.scene_event_failed";
                    diagnostic.Message = "A managed scene lifecycle event failed. Status " + std::to_string(status) + ".";
