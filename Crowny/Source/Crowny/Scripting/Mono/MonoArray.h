...
namespace Crowny
{
    ...
    namespace Detail
    {
        template <class T> T ScriptArray_Get(MonoArray* ar, uint32_t idx)
        {
        ...
        template <> inline ScriptArray ScriptArray_Create<uint8_t>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetByteClass(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<int8_t>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetSByteClass(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<char>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetCharClass(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<int16_t>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetI16Class(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<int32_t>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetI32Class(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<uint32_t>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetU32Class(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<int64_t>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetI64Class(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<uint64_t>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetU64Class(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<String>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetStringClass(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<Path>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetStringClass(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<float>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetFloatClass(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<double>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetDoubleClass(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<bool>(uint32_t size)
        {
            return ScriptArray(MonoUtils::GetBoolClass(), size);
        }

        template <> inline ScriptArray ScriptArray_Create<MonoObject*>(uint32_t size)
        {
