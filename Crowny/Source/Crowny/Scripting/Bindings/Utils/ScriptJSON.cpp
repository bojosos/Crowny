...
namespace Crowny
{
	...
	{
		// TODO: Fix this function. In the first if statement I look at C# types and the second one I look at json types.
		if (value.IsObject())
		{
	...
	static void FieldToJson(rapidjson::Value& cur, const Ref<SerializableTypeInfo>& typeInfo, void* data, rapidjson::Document& doc)
	{
		...
		else if (typeInfo->GetType() == SerializableType::Array)
		{
			...
			for (uint32_t i = 0; i < arr.Size(); i++)
			{
				...
				// FieldToJson(value, serializableArrayTypeInfo->m_ElementType, ScriptArray::GetArrayAddr(arr.GetInternal(), sizeof(MonoString), i), doc);
		...
		else if (typeInfo->GetType() == SerializableType::Object)
		{
			...
			// TODO: Fix this
			Ref<SerializableObjectInfo> objectInfo;
			if (ScriptInfoManager::Get().GetSerializableObjectInfo(serializableObjectInfo->m_TypeNamespace, serializableObjectInfo->m_TypeName, objectInfo))
