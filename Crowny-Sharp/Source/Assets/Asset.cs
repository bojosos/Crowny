...
namespace Crowny
{
    ...
    public struct UUID : IEquatable<UUID>
    {
        ...
        public static readonly UUID Empty = new UUID(0, 0, 0, 0);
        ...
        /// <summary>Returns whether the UUID has the same four words.</summary>
        public bool Equals(UUID other) => this == other;

        public override bool Equals(object obj) => obj is UUID && Equals((UUID)obj);

        internal static unsafe UUID FromBytes(byte* bytes)
        {
            return new UUID(ReadBigEndian(bytes), ReadBigEndian(bytes + 4), ReadBigEndian(bytes + 8), ReadBigEndian(bytes + 12));
        }

        private static unsafe uint ReadBigEndian(byte* bytes)
        {
