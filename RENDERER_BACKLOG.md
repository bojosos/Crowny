...
- [x] Remove redundant per-frame transient-resource hash nodes and string-dispatched pipeline callbacks. Frame-slot physical IDs and typed pass dispatch now keep warm Forward+/Deferred+ graph rebuilds and transient binding resolution allocation-free.
...
- [x] Weighted blended OIT no-device validation: focused Release coverage passed 598 assertions in 47 cases for GPU-draw grouping, graph resources/transitions, shader variations, and fallback selection. General 60-resource renderer regressions passed 5/5 Vulkan and OpenGL captures with all 5 cross-backend comparisons; those captures do not yet exercise weighted-OIT pixels.
- [ ] Add a deterministic overlapping-transparency renderer capture that checks weighted-OIT accumulation blending, revealage clear-to-one, composite math, and the OpenGL/compatibility fallback.
